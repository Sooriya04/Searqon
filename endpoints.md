...
| `POST` | `/api/v1/crawl` | Recursive crawl of an entire domain. Returns `jobId`. |
| `GET` | `/api/v1/crawl/:id` | Get status and results for a crawl job. |
| `POST` | `/api/v1/map` | Discover all internal URLs on a given domain. |
| `POST` | `/api/v1/extract` | Schema-based structured JSON extraction via LLM. |

## 🔍 Global Search
Unified search across web and specialized sources. Optimized for high speed and minimal resource usage.

| Method | Endpoint | Description |
| :--- | :--- | :--- |
| `POST` | `/api/v1/search` | General web search (DuckDuckGo based). |
| `POST` | `/api/v1/unified` | **Primary Unified Search Hub**. Returns flat JSON results. |
| `POST` | `/api/v1/classify` | Native intent classification (No Python/LLM needed). |

### `/api/v1/unified` [POST]
Returns a flat JSON array of search result objects: `[ { "title", "url", "content", "source" }, ... ]`.
- **Response Header**: `X-Search-Duration` provides the execution time in seconds (e.g., `3.45s`).
...
---
*Note: All search endpoints predominantly accept a `query` or `q` parameter in a JSON body.*
